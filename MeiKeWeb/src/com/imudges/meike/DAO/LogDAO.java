 ...
 * 
 * @see com.imudges.meike.bean.Log
...
public class LogDAO extends BaseHibernateDAO {
	...
	public Log findById(java.lang.Integer id) {
		...
		try {
			Log instance = (Log) getSession().get("com.imudges.meike.bean.Log",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Log instance) {
		...
		try {
			List results = getSession()
					.createCriteria("com.imudges.meike.bean.Log")
	...
	public List<Log> findByKeyword(String keyword) {
		...
		log.debug("finding Log List with keyword");
		try {
			String queryString = "from Log as model where model.logMessage like '%" + keyword + "%'";
			Query queryObject = getSession().createQuery(queryString);
			return queryObject.list();
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
