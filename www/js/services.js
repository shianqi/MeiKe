...
angular.module('starter.services', [])
      ...
      return {
        ...
        get : function(ClassId,ClassType){
          for (var i =0 ; i<items1.length;i++ ){
            if(items1[i].id === parseInt(ClassId)){
              return items1[i];
            }
          }
          return null;
        },
        gettype: function(ClassId){
          for (var i =0 ; i<items2.length;i++ ){
            if(items2[i].id === parseInt(ClassId)){
              return items2[i];
            }
          }
          return null;
    ...
    .service('LoginService', function ($q, $http) {
      ...
      return {
        ...
        //修改用户信息
        setPerson: function (email,username) {

          var deferred = $q.defer();
          var promise = deferred.promise;
          //ajax请求
          $http.jsonp(URL1+"changeUserInfo.action?email=" + email + "&username=" + username + URL2)
            //*******************************************
              .success(function (response) {
                if (response.status == 1) {
                  deferred.resolve('register successfully');
        ...
        submitFix: function (password1,password2,password3) {

          var deferred = $q.defer();
          var promise = deferred.promise;
          //ajax请求
          $http.jsonp(URL1+"changePassword.action?oldPassword=" + password1 + "&newPassword=" + password2+ "&repeatPassword=" + password3 + URL2)
            //*******************************************
              .success(function (response) {
                if (response.status == 1) {
                  deferred.resolve('register successfully');
